 	...
 	$.fn.extend({ 
 		imgPicker: function(options) {
	    	...
	    	, html5Upload = function(button) {
				...
				if (file.type.match(imageType) && file.type != 'image/bmp' && file.size <= 2000000) {
					...
					alertMessage('File not allowed Max 2 Mb , JPEG/JPG/PNG', 2);
	    	}
				...
				$(this).on('click', function() {
					...
					$('.ip-upload .ip-file').on('change', function() {
						...
						if (window.File && window.FileReader 
						 	&& window.FileList && window.Blob && options.html5Upload)
						 	html5Upload( $(this) );
						else 
						 	iframeUpload( $(this) );
		            });
