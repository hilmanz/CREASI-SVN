...
class talentskeerHelper {
	...
	function getTelentskeer(){
			...
			$sql = "SELECT sm.*,DATE_FORMAT(sm.register_date,'%d-%m-%Y %H:%i') AS tanggal,
					...
					WHERE 1 {$wSearch} {$wstartdate} {$wenddate} and sm.role !=1 and sm.n_status!=2  ORDER BY sm.love_count DESC LIMIT  {$start},{$limit}";
			// pr($sql);
			$this->logger->log($sql);
			...
			FROM social_member sm
					...
					LEFT JOIN 
					 ...
					 LEFT JOIN 
					 tbl_talent_seeker tblts ON sm.id=tblts.user_id
			WHERE 1 {$wSearch} {$wstartdate} {$wenddate} and sm.role !=1 and sm.n_status!=2  ";
			// pr($sql);
