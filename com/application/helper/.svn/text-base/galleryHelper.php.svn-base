...
class galleryHelper {
	...
	function getportofolio($uid,$type){
		global $CONFIG;
		...
		$sql = "SELECT *
			...
			WHERE user_id='{$uid}' and type='{$type}' and n_status!=2 order by id DESC";
	...
	function selectupdatedata($uid){
		...
		$sql = "SELECT *
			FROM {$CONFIG['DATABASE'][0]['DATABASE']}.my_portofolio
			WHERE user_id='{$uid}' and type='1' and n_status=1 order by id DESC";
			//pr($sql);exit;
		$res = $this->apps->fetch($sql,1);
	...
	function addgalery($filephoto,$uid){
		...
		//pr($filephoto);exit;
	//	PR('ss');exit	;	
		$photo = unserialize($filephoto);
		$array_photo=$photo['halaman_gallery'];
		//pr($array_photo);exit;
		
					foreach($array_photo as $key)
					{
						
						$sql = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.my_portofolio (`images`,`user_id`,`date`,`n_status`) 
								VALUES ('{$key}','{$uid}',now(),1)";
								$res = $this->apps->query($sql);
					}
						return $res;
				}
