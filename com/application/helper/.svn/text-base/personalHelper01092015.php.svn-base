...
class personalHelper {
	...
	function editguardian(){
		...
		$sql = "SELECT * FROM {$CONFIG['DATABASE'][0]['DATABASE']}.social_member where id='{$bioid}'";
		$fecth = $this->apps->fetch($sql);
		// pr($fecth['name']);
	   if($fecth)
	   {
			
			 $sqlCheck = "SELECT * FROM {$CONFIG['DATABASE'][0]['DATABASE']}.gm_notifikasi where user_id='{$bioid}' and title='Update Guardian'";
			...
			$iduser=$fecth['id'];
			$message='<a href="'.$CONFIG['ADMIN_DOMAIN'].'personalmanagement/detail?id='.$fecth['id'].' "> '.$fecth['name'].' </a> has update guardian information'; 
