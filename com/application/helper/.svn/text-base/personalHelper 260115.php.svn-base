...
class personalHelper {
	...
	function editbiopersonalct(){
		...
		$sql = "update social_member set `nickname`='{$nickname}',`phone_number`='{$telp}',`email`='{$email}',`testimoni`='{$testimoni}' where id='{$bioid}'";
	...
	function selectcategory($id){
		...
		if($rqData) {
			...
			foreach($rqData as $key => $val){
				...
				$arrayfetchr= array();
				$sql="select * from my_portofolio where category_id='{$val['category_id']}' and user_id={$val['user_id']}";
				//pr($sql);exit;
