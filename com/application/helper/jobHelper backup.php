...
class jobHelper {
   ...
   function checkinterview()
	...
	function detail_talent_jobs($id,$param=null)
	{
	global $CONFIG;
 ...
 on jb.id_job=jc.jobboard_id
	...
	//	pr($subcategory);exit;
		...
		$sql = "INSERT INTO {$CONFIG['DATABASE'][0]['DATABASE']}.jobboard (`job_title`,`talent_seeker_id`,`job_description`,`requitment`,`salary`, `status_jobs`,`date`,`n_status`,`provinsi`,`city`,`enddate`,`experience`,`agent`,`file`) 
							VALUES ('{$title}','{$ts_id}', '{$description}','{$requitment}','{$salary}', 0,'{$datenya}',6,'{$provinsi }','{$city}','{$endpublish}','{$experience}','{$agent}','{$filenamenya}')";
		//pr($sql);exit;		  	
		$res = $this->apps->query($sql);
	...
	global $CONFIG;
		...
		if ($id == 'last')
		{
		$detailid = " order by jb.date DESC";
		}else if ($id == 'end')
		{
...
$joinrequesti= " left join tbl_category tc on jc.category_id=tc.id left join tbl_subcategory ts on jc.subcategory_id=ts.id ";		
		...
		//RUN FILTER
		$filter = "";
		$filter = $search=="Search..." ? "" : "AND (tts.nama_perusahaan LIKE '%{$search}%' )";
		// $filter .= $notiftype!=0 ? " AND notiftype = {$notiftype}" : " AND notiftype = 3";
		// $filter .= $publishedtype ? "AND n_status = {$publishedtype}" : " AND n_status != 3";
		...
		$sql = "SELECT count(*) total
			from {$CONFIG['DATABASE'][0]['DATABASE']}.jobboard as jb left join tbl_talent_seeker tts on jb.talent_seeker_id=tts.id left join job_category jc
 on jb.id_job=jc.jobboard_id{$joinsortirapp} {$jcheckadvsearch} WHERE 1  {$requesti} AND jb.n_status=1 {$checkadvsearch} {$cetegorynya} {$filter}  {$detailid}";
	...
	//pr($sql);exit;
		...
		$sql = "select *,DATE_FORMAT(jb.enddate,'%d-%m-%Y') as date,jb.provinsi,SUBSTRING(jb.job_description,1,150) AS deskripsi,jb.city{$ssortirapp} from {$CONFIG['DATABASE'][0]['DATABASE']}.jobboard as jb left join tbl_talent_seeker tts on jb.talent_seeker_id=tts.id left join job_category jc
 on jb.id_job=jc.jobboard_id{$joinsortirapp} {$joinrequesti} {$jcheckadvsearch} WHERE 1  {$requesti} AND jb.n_status=1 {$checkadvsearch} {$cetegorynya} {$filter} group by jb.job_title {$detailid} {$qlimit}";
		//pr($sql);exit;
		$rqData = $this->apps->fetch($sql,1);
		//pr($rqData);exit;
		if($rqData=='')
