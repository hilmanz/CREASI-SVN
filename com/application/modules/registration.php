...
class registration extends App{
	...
	function save(){
		...
		if($lastname=='')
		{
			$lastname_no="You cannot leave this field empty";
			$pesanError="ada";
		}
		if($Date_Month=='')
		{
		...
		if($cechkemail)
		{
			$email_no=" The Email has already been registered!";
			$pesanError="ada";
		}
		...
		if($pesanError)
		{
			...
			$this->assign('lastname_no', $lastname_no);
	...
	function checkemail(){
		...
		if($cechkemail)
		{
			print_r( json_encode(array('status'=>true)));die;
		}
		else
		{
	...
	function thanksreg(){
		...
		return $this->View->toString(TEMPLATE_DOMAIN_WEB .'apps/emailverif.html');
