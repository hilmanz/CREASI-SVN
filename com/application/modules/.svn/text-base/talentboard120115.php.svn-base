...
class talentboard extends App{
	...
	function main(){
		...
		$listpersonal = $this->talentHelper->listtalent ($start=null,$limit=12);
	...
	//pr($_POST);
		...
		//pr($_POST);exit;
		$uidlike=encrypt(@$this->user->id);
		$ajax = $this->talentHelper->listtalent($start=null,$limit=12,$sorting);
		$popular = $this->talentHelper->popular();
	...
	function ajaxPaging(){
		...
		//pr('sas');
			$ajax =	$this->talentHelper->listtalent($start,$limit=12,$sorting);
		}else if($this->_p('ajax') ){
		//pr('ss');
			$ajax =	$this->talentHelper->listtalent($start,$limit=12);
		}
		$uidlike=encrypt(@$this->user->id);
			$popular = $this->talentHelper->popular();
		//pr($ajax);
