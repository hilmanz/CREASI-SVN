...
class personal extends App{
	...
	function main(){
		...
		$categorynya = $this->categoryHelper->listcategory();
		//pr($citynya);exit;
		$this->assign('citynya', $citynya);
		$selectcategory = $this->personalHelper->selectcategory($this->user->id);
	// pr($selectcategory);die;
		$selectcategorydetail = $this->personalHelper->selectcategorydetail($this->user->id);
		...
		$selectlike = $this->personalHelper->selectlike($this->user->id);
		// pr($selectlike);exit;
		$this->assign('category', $categorynya);
		//$selectexperience = $this->personalHelper->selectexperience($this->user->id);
		// pr($selectcategory['result']['category']);exit;
		...
		$this->assign('listcategory', $category);
		$this->assign('selectcategory', $selectcategory['result']);
		$this->assign('selectcategorynew', $selectcategory['result']['category']);
		$this->assign('selectcategorydetail', $selectcategorydetail['result']);
	...
	function save(){
	global $LOCALE,$CONFIG;
	$resultcategory = $this->personalHelper->addcategory($this->user->id);
	if($resultcategory)
	{
	...
	function editpersonalts(){
		...
		if(isset($_POST['bioinput']))
		{
			...
			print json_encode(array('status'=>true,'name'=>$name,'brand'=>$brand,'provincy'=>$provincy,'city'=>$city,'fb'=>$fb
									,'twitter'=>$twitter,'youtube'=>$youtube,'instagram'=>$instagram,'website'=>$website,'lastname'=>$lastname)); exit;
			}
		}
		if(isset($_POST['companyinf']))
		{
			...
			if($companyinf == true){
			print json_encode(array('status'=>true,'address'=>$address,'brand'=>$brand,'name'=>$name,'lastname'=>$lastname,'companyinf'=>$companyinf,'email'=>$email,'phone'=>$phone,'testimoni'=>$testimoni,'phonecpok'=>$phonecpok)); exit;
			}
		}
			...
			if($countaudio['total'] >= 20)
			{
				print_r(json_encode(array('status'=>0,'msg'=>'maximum 20 portfolio')));exit;
						...
						$domain = parse_url($video_url);
							...
							{
								// pr('ooo');
								$explodevimeo = explode('/',$video_url);
								$counexlode=count($explodevimeo);
								$id_video_youtube =$explodevimeo[$counexlode-1];
								/* preg_match(
										'/vimeo\.com\/([0-9]{1,10})/',
										$video_url,
										$matches
									...
									{
										preg_match(
											'/https?:\/\/(?:www\.)?vimeo.com\/(?:channels|originals\/(?:\w+\/)?|groups\/([^\/]*)\/videos\/|inthemoment/|album\/(\d+)\/video\/|)(\d+)(?:$|\/|\?)/',
											$video_url,
											$matchess
										);
										
										
										$id_video_youtube =@$matchess[3];
										$refrence='vimeo';
									}*/
							}
						// pr($id);die;
			...
			if(isset($_POST['educations']))
	{
		...
		{
			// pr($_POST);exit;
			$editcategory = $this->personalHelper->editcategory($this->user>id);
	...
	function ajaxcrop()
	{
		global $LOCALE,$CONFIG;
		// echo"ssss";die;
			$uploadpicture = $this->uploadHelper->imagepicter2();
			//pr($uploadpicture);die;
	}
	function ajaxcropcover()
	{
		global $LOCALE,$CONFIG;
		
			$uploadpicture = $this->uploadHelper->imagepicter3();
		//	pr($uploadpicture);die;
	}
	...
	function ajaxfotoprofile()
	{
		...
		// }
		$filenamenya=$this->_p('fotoavatar');
	...
	function deleteedu() {
		...
		//header('Content-Type: application/json');
	  	$id = intval($this->_request('id'));
		$uid=$this->user->id;
		$catdelete = $this->personalHelper->deleteedu($id,$uid);
		if($catdelete==true)
		{
			sendRedirect($CONFIG['BASE_DOMAIN']."personal");
			return $this->out(TEMPLATE_DOMAIN_WEB . '/login_message.html');
	...
	function deleteexperience() {
		...
		//header('Content-Type: application/json');
	  	$id = intval($this->_request('id'));
		$uid=$this->user->id;
		$catdelete = $this->personalHelper->deleteexperience($id,$uid);
		if($catdelete==true)
		{
			sendRedirect($CONFIG['BASE_DOMAIN']."personal");
			return $this->out(TEMPLATE_DOMAIN_WEB . '/login_message.html');
