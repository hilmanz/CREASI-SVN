...
class markingHelper {
	...
	function getuserMarking(){
			...
			$sql = "SELECT sm.*,DATE_FORMAT(sm.register_date,'%d/%m/%Y') AS tanggal,
					...
					WHERE 1 AND sm.role !=2 AND mya.marking !=0  GROUP BY mya.user_id LIMIT  {$start},{$limit}";
			// pr($sql);die;
			$this->logger->log($sql);
			...
			FROM social_member sm
					...
					WHERE 1 AND sm.role !=2 AND mya.marking !=0 GROUP BY mya.user_id";
