...
class personalhierHelper {
	...
	function getuserhier(){
			...
			$sql = "SELECT sm.*,DATE_FORMAT(sm.register_date,'%d/%m/%Y') AS tanggal,
					mya.marking,jb.job_title,ts.nama_perusahaan
						
					FROM social_member sm
					
					LEFT JOIN 
					 tbl_talent tblta ON sm.id=tblta.user_id
					 LEFT JOIN 
					 my_application mya ON sm.id=mya.user_id
					 LEFT JOIN
					 jobboard jb ON jb.id_job= mya.jobboard_id
					 LEFT JOIN 
					 tbl_talent_seeker ts ON jb.talent_seeker_id=ts.id
					 
					WHERE 1 AND sm.role !=2 AND mya.hire =1  GROUP BY mya.user_id LIMIT  {$start},{$limit}";
			// pr($sql);die;
			$this->logger->log($sql);
			...
			FROM social_member sm
					...
					WHERE 1 AND sm.role !=2 AND mya.hire =1  GROUP BY mya.user_id";
	...
	function upmarking(){
			...
			$result['status']=0;
			$markingstatus=$this->apps->_p('markingstatus');
			...
			$sql = "UPDATE tbl_talent 
					SET marking='{$markingstatus}' 
					WHERE user_id='{$idCt}'";
			// pr($sql);
			$this->logger->log($sql);
	...
	function detailcreativetalent(){
		global $CONFIG;
			...
			$data['profile']=$res;
			$sqlsportofolio = "SELECT * FROM {$CONFIG['DATABASE'][0]['DATABASE']}.my_portofolio where user_id='{$res['user_id']}' limit 1 
						";
