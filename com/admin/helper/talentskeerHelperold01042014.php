...
class talentskeerHelper {
	...
	function getjobs(){	
		...
		$sql = "
			SELECT jb.*,DATE_FORMAT(jb.date,'%d/%m/%Y %h:%i %p') AS DATE,ts.nama_perusahaan AS namaPerusahaan,tc.category_name,tsub.name_subcategory
			FROM jobboard as jb
	...
	$idjobs=$this->apps->_request('idjobs');
		$sql = "
			SELECT *,DATE_FORMAT(jobboard.enddate,'%d-%m-%Y') as enddate,jobboard.city as city,jobboard.provinsi as provinsi
			FROM jobboard 
