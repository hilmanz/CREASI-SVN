...
class guardianHelper {
	...
	function getuserguardian(){
			...
			$sql = "SELECT sm.*,YEAR(CURDATE()) - YEAR(sm.birthday) AS age ,mg.name_parent,mg.address,mg.phone,mg.email
					FROM social_member sm
					LEFT JOIN 
					my_guardian mg ON mg.user_id=sm.id
					WHERE YEAR(CURDATE()) - YEAR(sm.birthday)<=18 and sm.role!=2
					LIMIT  {$start},{$limit}";
			// pr($sql);die;
			$this->logger->log($sql);
