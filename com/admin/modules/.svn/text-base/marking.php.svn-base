<?php
class marking extends App{
		
	function beforeFilter(){ 
		global $LOCALE,$CONFIG; 
		
		$this->markingHelper = $this->useHelper("markingHelper");
	...
	function main(){
		$dataCT = $this->markingHelper->getuserMarking();
		// pr($dataCT);die;
		$time['time'] = '%H:%M:%S';
		$this->assign('list', $dataCT['data']);
		$this->assign('total', $dataCT['total']);
		return $this->View->toString(TEMPLATE_DOMAIN_ADMIN .'apps/markingmanagement.html');
